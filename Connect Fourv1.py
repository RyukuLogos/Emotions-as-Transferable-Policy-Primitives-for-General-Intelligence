...
class ConnectFour:
    ...
    def get_legal_actions(self):
        ...
        for col in range(self.cols):
            if self.board[0, col] == 0:  # 最上段が空いている
                legal.append(col)
        return legal
    ...
    def render(self):
        ...
        for i, row in enumerate(self.board):
            print(f'{i} ' + ' '.join(symbols[int(x)] for x in row))
        print()
...
class Encoder(nn.Module):
    ...
    def __init__(self, z_dim=32):
        ...
        # 行動埋め込み（0-6の列 + 初期値7）
        self.action_embed = nn.Embedding(8, 16)
...
class AdvantageActorCriticMotor(nn.Module):
    ...
    def __init__(self, z_dim=32, num_emotions=4, num_actions=7):
        super().__init__()
        self.emotion_embed = nn.Embedding(num_emotions, 16)
...
class EmotionalAgent:
    def __init__(self, device='cuda', z_dim=32, num_emotions=4, lr=1e-3):
        ...
        self.motor = AdvantageActorCriticMotor(z_dim, num_emotions, num_actions=7).to(device)
    ...
    def self_play_episode(self):
        ...
        a_prev = torch.tensor([7]).to(self.device)  # 初期値（7列目より外）
        ...
        while not game.done:
            ...
            action_probs_masked = torch.zeros(7).to(self.device)
    ...
    def evaluate(self, num_games=50):
        ...
        for _ in range(num_games):
            ...
            z = torch.zeros(1, self.z_dim).to(self.device)
            a_prev = torch.tensor([7]).to(self.device)
...
def main():
    ...
    print("=" * 60)
    print("Connect Four with Emotional AGI")
    print("=" * 60)
    print("Game: 6×7 board, 4-in-a-row to win")
    print("Architecture:")
    print("  - z_dim: 32")
    ...
    for iteration in range(100):
        ...
        if iteration % 10 == 0:
            ...
            print(f"Iter {iteration:3d}: Self-play {results} | vs Random: W={wins:2d} D={draws:2d} L={losses:2d} ({win_rate:.1f}%)")
    
    print()
    print("Training complete!")
    print("Final evaluation (100 games)...")
    wins, draws, losses = agent.evaluate(num_games=100)
    win_rate = wins / 100 * 100
    print(f"Final: W={wins} D={draws} L={losses} ({win_rate:.1f}% win rate)")
